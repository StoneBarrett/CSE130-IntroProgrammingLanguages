...
// randomly selecting contacts, deleting all contacts, saving phonebooks, and opening previous phonebooks, and use 
...
void open()
{
	...
	{
		pRead = fopen("contacts.txt", "r");
	}
	
	if (pRead == NULL)
	{
		printf("Phonebook could not be opened.\n");
	}
	else
	{
		listVal = 0;
		...
		while (!feof(pRead))
		{
			...
			else
			{
				...
				fscanf(pRead, "%s %s %s\n", cl[listVal].firName, cl[listVal].lasName, cl[listVal].number);
...
void save()
{
	...
	{
		pWrite = fopen("contacts.txt", "w");
	}
	
	if (pWrite == NULL)
	{
		printf("Phonebook could not be saved.\n");
	}
	else
	{
		int loopVal;
